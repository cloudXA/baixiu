...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    if (isset($data[0][0])) {
        ...
        echo json_encode(array(
            'success' => false,
            'message' => 'option key does not exit'
        ));
    }
    exit;
}
...
// 判断是否存在该属性
$exit = xiu_query(sprintf("select count(1) from `options` where `key` = '%s'",$_POST['key']))[0][0] > 0;

if ($exit) {
    $affected_rows = xiu_query(sprintf("update `options` set `value` = '%s' where `key` = '%s'",$_POST['value'],$_POST['key']));
